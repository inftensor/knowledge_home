...
Status ELinkListLocate(ELinkList L, ElemType e,EPosition *q, Status(Compare)(ElemType e1, ElemType e2)) {
    // 确保存在
    if (!L.head) {
        return ERROR;
    }
    ...
    *q = NULL;
    ...
    while (cur && Compare(cur->data, e)) {
        pre = cur;
        cur = cur->next;
    }

    if (cur && Compare(cur->data, e)==0) {
        *q = cur;
        return TRUE;
    }
    *q = pre;
