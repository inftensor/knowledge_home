...
> @Definition
>
> **定义1.1**	设$a$和$\delta$是两个实数，且$\delta>0$,集合$\{x\in R| |x-a|\lt\delta\}$称为点$a$的$\delta$邻域，记为$U(a,\delta)$,点$a$称为此邻域的中心，$\delta$称为此领域的半径。集合${x\in R|0\lt|x-a|<\delta}$称为点$a$的$\delta$去心邻域。通常，集合$\{x\in R| a-\delta\lt x\lt a\}$称为点$a$的$\delta$的左邻域；集合$\{x\in R| a\lt x\lt a+\delta\}$称为点$a$的$\delta$右邻域。
> 具体如下图：
...
> @Theorem
>
> **定理3.4**	设$\lim_{x\to x_0}f(x)$存在，则函数$f(x)$在点$x_0$的某一去心领域内有界。
